...
class TwoWayAttentionBlock(nn.Module):
    def __init__(
        ...
        self.global_query = nn.parameter.Parameter(data=0.1 * torch.randn(1, 50, embedding_dim))
    ...
    def forward(self, img_embed, point_embed, img_pe, point_pe) -> Tuple[Tensor, Tensor]:
        ...
        # point_embed[:, :10, :] = point_embed[:, :10, :] + self.global_query
        # q = point_embed
        ...
        # attn_out = self.cross_attn_token_to_image(q=queries, k=img_embed, v=img_embed)
        # queries = self.norm2(attn_out)
