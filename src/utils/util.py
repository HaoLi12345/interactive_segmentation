...
def save_predict(args, logger, final_pred, seg, pred, points_dict, idx, test_data, image_data, patient_name):
    """Save prediction NIfTI files. Ported from lab promise/src/utils/util.py.

    Only invoked when ``args.save_predictions`` is True.
    """
    import os
    ...
    x_dimension, y_dimension, z_dimension = (
        points_dict["x_dimension"], points_dict["y_dimension"], points_dict["z_dimension"],
    )
    ...
    save_prediction_path = os.path.join(
        save_predict_dir, patient_name.replace(".nii.gz", "_prediction.nii.gz")
    )
    save_image(pred, test_data, image_data, save_prediction_path)

    seg_points = torch.zeros_like(seg).to(pred.device)
    seg_points[0, 0, new_z, new_y, new_x] = 1
    save_point_path = os.path.join(
        save_predict_dir, patient_name.replace(".nii.gz", "_point.nii.gz")
    )
    save_image(seg_points, test_data, image_data, save_point_path)

    save_probability_path = os.path.join(
        save_predict_dir, patient_name.replace(".nii.gz", "_probability.nii.gz")
    )
    save_image(final_pred, test_data, image_data, save_probability_path)

    logger.info(
        "- Case %s prediction saved (x=%d y=%d z=%d)",
        test_data.dataset.img_dict[idx],
