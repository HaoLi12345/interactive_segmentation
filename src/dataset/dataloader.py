...
class Dataset_promise(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.split == "train":
            ...
            save_dict = {"image": subject_save.image.data.clone().detach(), "label": subject_save.label.data.clone().detach()}
            return img_aug, subject.label.data.clone().detach(), self.image_paths[index], save_dict
    ...
    def _set_file_paths(self, data_dir, split):
        ...
        max_cases = getattr(self.args, "max_cases", 0) or 0
        if max_cases == 0 and getattr(self.args, "use_small_dataset", False):
            max_cases = 10
        if max_cases > 0:
            self.image_paths = self.image_paths[:max_cases]
            self.label_paths = self.label_paths[:max_cases]
